...
export class PluginUpdater {
    ...
    async fetchFromPinsonBots() {
        try {
            ...
            return {
                ...
                format: 'tar.gz' // PinsonBots always uses tar.gz
            };
        }
        catch (error) {
            console.warn('[Updater] PinsonBots unavailable:', error.message);
            return null;
        }
    }
    /**
    ...
    async fetchFromGitHub() {
        try {
            ...
            return {
                ...
                format: 'zip' // GitHub uses zip
            };
        }
        catch (error) {
            console.error('[Updater] GitHub fetch failed:', error.message);
            return this.getCachedVersion();
        }
    }
    /**
    ...
    async cleanup(archivePath, tempDir) {
...
export function getUpdater(projectRoot, options) {
    if (!updaterInstance || projectRoot) {
        updaterInstance = new PluginUpdater(projectRoot || process.cwd(), options);
    }
